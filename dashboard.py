...
function showApproveModal(btn) {
  ...
  cb.textContent = '✓ Approve for ' + day; cb.disabled = false;
...
async function confirmApprove() {
  ...
  try {
    ...
    if (!getResp.ok) { var ge = await getResp.json().catch(function(){ return {}; }); throw new Error('Could not read draft (' + getResp.status + '): ' + (ge.message || 'check PAT')); }
    var fileData = await getResp.json();
    var postJson = JSON.parse(atob(fileData.content.replace(/\n/g,'')));
    postJson.approved = true; postJson.status = 'approved';
    postJson.approval_required = false; postJson.approved_at = new Date().toISOString();
    postJson.dry_run = false;
    var encoded = btoa(unescape(encodeURIComponent(JSON.stringify(postJson, null, 2))));
    cb.textContent = 'Saving…';
    var putResp = await fetch(
      'https://api.github.com/repos/' + _REPO + '/contents/' + path,
      { method: 'PUT',
        headers: { 'Authorization': 'Bearer ' + pat, 'Accept': 'application/vnd.github.v3+json', 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: 'approve: ' + path.split('/').pop(), content: encoded, sha: fileData.sha, branch: _BRANCH }) }
    );
    if (putResp.status === 200 || putResp.status === 201) {
      var day = (_approveSourceBtn && _approveSourceBtn.getAttribute('data-publish-day')) || 'scheduled day';
      ...
      st.textContent = 'Error ' + putResp.status + ': ' + (pe.message || 'check PAT scope');
      st.style.color = '#e8372a'; cb.textContent = '✓ Approve'; cb.disabled = false;
    }
  } catch(e) { st.textContent = 'Error: ' + e.message; st.style.color = '#e8372a'; cb.textContent = '✓ Approve'; cb.disabled = false; }
}
...
<div class="modal-overlay" id="approve-modal" role="dialog" aria-modal="true" aria-labelledby="approve-modal-title">
  <div class="modal-box">
    ...
    <div class="modal-field">
      ...
      <input class="modal-input" id="modal-pat" type="password" placeholder="ghp_..." autocomplete="off" aria-describedby="modal-pat-hint">
    ...
    <div class="modal-actions">
      <button class="modal-confirm approve-green" id="modal-confirm-btn" onclick="confirmApprove()">&#10003; Approve</button>
